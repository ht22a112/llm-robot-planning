...
task_additional_info:（任意）必要なら補足情報を記述してください。
task_dependencies:（任意）このタスクが依存する他のタスクとその理由、および必要なアウトカムをリスト化してください。
    dependency_task_sequence_number: 依存するタスクの番号
    reason: 理由
...
task_outcome:（必須）タスクの完了条件を明示してください。以下の2つの情報を含めます：
    ...
    desired_robot_state:（任意）タスク完了時にロボットが持つべき状態（以下の候補一覧から選択してください）。
指示:
「{{instruction}}」

desired_robot_state の候補一覧:
- in_hand(x: str) : ロボットがxを手に持っている
- not_in_hand() : ロボットが何も手に持っていない
  ...
  "tasks": [
    {
      ...
      "task_environmental_conditions": {
        ...
        "required_information_conditions": ["string"]
      },
      "task_reason": "string",
      "task_outcome": {
        "desired_information": [
